...
theorem interface_execution (api : Interface M Input Output) {p : Program M}
    {P : State → Prop} {Q : State → State → Prop} {budget : State → Nat}
    (proof : Correct p P Q budget) (input : Input) (valid : P (api.initial input)) :
    ∃ final, Checked.Exec (Cmd.seq api.prepare p.source).compile (encode (api.encode input))
...
theorem method_execution {api : Interface M Input Output} (p : VerifiedMethod api)
    ...
    ∃ final, Checked.Exec (Cmd.seq api.prepare p.method.body.source).compile
