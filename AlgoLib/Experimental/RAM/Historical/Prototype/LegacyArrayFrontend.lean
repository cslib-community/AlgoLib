...
import AlgoLib.Experimental.RAM.Language.Model.Mutable
import AlgoLib.Experimental.RAM.Language.Model.MultipleArrays
import AlgoLib.Experimental.RAM.Verification.Semantics.Procedures
...
programs. This explicitly named `legacy_ram` adapter remains only for the earlier
contiguous/indirect array substitution proofs and their negative regression tests.
It is never selected automatically. `prove_legacy_algorithm` and the compatibility
`prove_ram` command preserve those existing checked artifacts during migration.

New programs should import Composition and use `ram method` / `prove_algorithm`.
-/
...
syntax "legacy_ram" "method" ident leafny_binder* "return" "(" ident ":" term ")"
...
elab_rules : command
  | `(command| legacy_ram method $name:ident $binders:leafny_binder*
      return ($ret:ident : $retTy:term)
    ...
    unless binders.all (fun b => match b with
      | `(leafny_binder| (mut $_:ident : Array Nat)) => true
      | _ => false) do throwError "RAM inputs must be declared mutable arrays"
...
syntax "prove_legacy_algorithm" ident "by" tacticSeq : command
elab_rules : command
  | `(command| prove_legacy_algorithm $name:ident by $proof:tacticSeq) => do
