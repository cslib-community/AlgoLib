...
-/
import AlgoLib.Experimental.RAM.Library.Graph.Traversal
import AlgoLib.Experimental.RAM.Implementations.Natural.Memory.GraphMemory
import AlgoLib.Experimental.RAM.Historical.Backend.Certificates.LoopVC
import Mathlib.Tactic

/-!
# Graph traversal implementation certificates

Proves discovery, row scanning, and frontier facts together with instruction-level graph
operations and cost bounds.

The search adapter exposes certified actions; Library/Search summarizes a row traversal;
Programs/Connectivity proves the canonical client loop.
