...
-/
import AlgoLib.Experimental.RAM.Historical.NatMachine.Machine

/-!
# Historical natural-machine reference evaluator

This evaluator and its execution theorem are retained for regression comparisons
and the Nat-to-Int migration proof. Supported programs execute through
`Integer.run`, `Integer.TotalProgram`, or the native typed method compiler.

The reference evaluator uses well-founded recursion on the machine transition
relation. Its termination proof is erased by Lean; it does not use fuel search or
noncomputable choice of output. The obsolete Nat `TotalProgram` wrapper has been
removed so it cannot be mistaken for the supported execution API.
-/
