...
/-!
# Fuel-free RAM runner

Executes RAM code using a termination certificate and proves agreement with the declarative
counted execution relation.

The certificate is generated by higher-level contracts. End users provide input and its
precondition, not a guessed fuel value.
...
def step (c : Config) : Option (Nat × Config) :=
  match c.todo with
  | [] => none
  | .block is :: rest => (blockEval is c.state).map fun t => (is.length, ⟨rest, t⟩)
...
theorem Exec.trace {a : Code} {s t : State} {i : Nat} (h : Exec a s i t)
  ...
  | block hs => exact .next (by simp [step, hs]) ht
  | seq ha hb iha ihb =>
...
def execute (c : Config) (h : Acc Next c) : Option (Nat × State) :=
  match hs : step c with
  | none => if c.todo.isEmpty then some (0, c.state) else none
  | some (k, d) =>
    let r := execute d (h.inv ⟨k, hs⟩)
    r.map fun result => (k + result.1, result.2)
...
theorem execute_eq {c : Config} {k : Nat} {t : State} (ht : Trace c k t)
    (h : Acc Next c) : execute c h = some (k, t) := by
  induction ht with
...
/-- A successful execution supplies both accessibility and evidence of a result. -/
def run (c : Code) (s : State) (h : Terminates c s) : Nat × State :=
  let acc : Acc Next ⟨[c], s⟩ := by
    obtain ⟨k, t, hx⟩ := h
    exact (hx.trace (.done t)).accessible
  (execute ⟨[c], s⟩ acc).get (by
    obtain ⟨k, t, hx⟩ := h
    rw [execute_eq (hx.trace (.done t))]
    rfl)

theorem run_eq {c : Code} {s t : State} {k : Nat} (hx : Exec c s k t)
    (h : Terminates c s) : run c s h = (k, t) := by
  simp [run, execute_eq (hx.trace (.done t))]
...
def TotalProgram.run (p : TotalProgram) (s : State) : Nat × State :=
  Integer.run p.code s (p.terminates s)

theorem TotalProgram.run_correct (p : TotalProgram) (s : State) :
    Exec p.code s (p.run s).1 (p.run s).2 := Integer.run_correct _ _ _
