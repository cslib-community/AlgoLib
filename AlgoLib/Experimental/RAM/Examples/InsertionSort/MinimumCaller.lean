...
-/
import AlgoLib.Experimental.RAM.Examples.InsertionSort.Proofs

/-!
# Verified sorting and a modular caller

The paper program lives in `Examples/InsertionSort/Program`, its generated API in `Examples/InsertionSort/Obligations`;
edit algorithmic proofs in
`Examples/InsertionSort/Proofs`. `Examples/InsertionSort/Backend` depends only on the specification, so proof edits
reuse its instruction certificates. `Examples/InsertionSort/Execution` connects proofs to that backend.
