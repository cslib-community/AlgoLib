...
namespace AlgoLib.Experimental.RAM.Algorithms.Examples
...
def report (graph : EdgeInput) (source : Nat) (hs : source < graph.n) : List Nat × Nat :=
  let r := Algorithms.BFS.run (graph.fromSource source hs)
...
example (graph : EdgeInput) (s : Nat) (hs : s < graph.n) (v : Nat) :
    (Algorithms.BFS.run (graph.fromSource s hs)).visited.contains v = true ↔
      Reachable graph.graph s v := Algorithms.BFS.run_correct _ v

example (graph : EdgeInput) (s : Nat) (hs : s < graph.n) :
    (∀ v ∈ graph.graph.vertexSet,
      (Algorithms.BFS.run (graph.fromSource s hs)).visited.contains v = true) ↔
        Connected graph.graph := Algorithms.BFS.connected_iff _
...
end AlgoLib.Experimental.RAM.Algorithms.Examples
