...
Start with [MixedAlgorithms.lean](../Examples/Composition/MixedAlgorithms.lean) for a small example, then
[Sorting.lean](../Examples/InsertionSort/MinimumCaller.lean) for insertion sort with both loops visible. These files
...
5. **Prove the generated obligations.** Run `generate_obligations name`, inspect
   ...
   and RAM compilation. See [the generated API tutorial](../Verification/OBLIGATION-API.md).
6. **Use the supplied runner.** For insertion sort:

   ```lean
   import AlgoLib.Experimental.RAM.Language.Owned.SortingExecution
...
|---|---|---|
| [LogicalFrontend](Frontend.lean) | One declaration and proof command | Code, contracts, invariants |
| [Frontend](Elaboration.lean) | Scope checking, typed paths, guard materialization, automatic frames and local hiding | Nothing additional |
| [Expressions](Expressions.lean) | Independent expression semantics, bounds and logical charges | Mathematical safety proofs |
| [Verification.Plan](../Verification/Plan.lean) | Indexed plans; calls use public summaries; total-correctness soundness | Generated source obligations |
| [Loom](../Verification/Loom.lean) | Actual Loom WP for the same program | The same source proof |
| [ExpressionImplementation](../Implementations/Natural/ExpressionImplementation.lean) | Ownership-directed expression certificates and updates | Nothing |
| [Storage](../Implementations/Natural/Storage.lean) | Verified scalar and array implementations | Nothing |
| [LocalImplementation](../Implementations/Natural/LocalImplementation.lean) | Private local initialization and scope boundaries | Nothing |
| [Encoding](../Implementations/Natural/Encoding.lean) | Compositional resident inputs, local layout reconstruction, fuel-free runner | Ordinary input values |
| [Linking](../Compiler/Assembly/Natural/Linking.lean) | Reconstruct composition certificates and invoke the existing RAM compiler | Nothing |
...
[LegacyArrayFrontend](../Historical/Prototype/LegacyArrayFrontend.lean) is available only through an
...
lexical rejection, unsafe indexing, computed call arguments, and paired calls. [SortingExecution](../Examples/InsertionSort/Execution.lean)
