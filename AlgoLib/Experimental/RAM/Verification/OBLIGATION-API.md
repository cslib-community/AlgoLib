...
These statuses depend on what your file imports. Importing `Examples/InsertionSort/Proofs.lean` instead
of `Examples/InsertionSort/Obligations.lean` brings in the finished proofs, so it is not the starting point for
...
responsibility. The actual [sorting proof file](../Examples/InsertionSort/Proofs.lean) shows the focused
...
For BFS, try this in a file importing `Examples/BFS/Obligations.lean` and opening its namespace:
...
`process_head` and `scan_work`. The existing [BFS proof blocks](../Examples/BFS/Proofs.lean)
...
Changing obligation automation in `Examples/InsertionSort/Obligations.lean` also reuses the backend. Changing the
program or its inline invariant annotations in `Examples/InsertionSort/Program.lean` rebuilds its dependents.
