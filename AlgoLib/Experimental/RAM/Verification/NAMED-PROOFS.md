...
Start with [insertion sort](../Examples/InsertionSort/MinimumCaller.lean) or [BFS](../Examples/BFS/Proofs.lean).
...
The standalone blocks in [SortingProofs.lean](../Examples/InsertionSort/Proofs.lean) are the recommended
...
[SortingFacts.lean](../Library/SortingFacts.lean). They state the familiar sorted-prefix
...
[BreadthFirstProgram.lean](../Examples/BFS/Program.lean) has three named loops: `clear`, `search`, and
...
No fuel is required. [BFSExecution.lean](../Examples/BFS/Execution.lean) similarly assembles
`BreadthFirst.search` and its graph-correctness and linear-time theorems for each
registered FIFO backend. See [OWNED-BFS.md](../Examples/BFS/TUTORIAL.md) for runnable graph inputs.
