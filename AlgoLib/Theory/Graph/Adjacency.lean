...
`AlgoLib.Graph.Basic` (`Graph`, `SimpleGraph`, `DiGraph`, `SimpleDiGraph`)
...
therefore place it directly above `AlgoLib.Graph.Basic` and below
`AlgoLib.Graph.Degree`, where the neighbour and degree functions are
...
* **One definition per graph type.** As elsewhere in `AlgoLib.Graph`, we
...
* **Adjacency is pure edge-existence; it does not exclude loops.** `Adj` is
  ...
  adjacency relation, so the `\ {v}` lives in `AlgoLib.Graph.Degree`.
