...
@[grind] noncomputable def graphExpansion (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)]
    (d : ℕ) : ℝ :=
...
@[grind] noncomputable def SimpleGraph.rayleighQuotient (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)]
    (x : α → ℝ) : ℝ :=
...
lemma rayleighQuotient_nonneg (G : SimpleGraph α)
...
@[grind] def orthogonalVectors (G : SimpleGraph α)
...
@[grind] def R_values (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)] : Set ℝ :=
  { r | ∃ x ∈ orthogonalVectors G, r = G.rayleighQuotient x }

@[grind] noncomputable def lambda2 (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)] : ℝ :=
    sInf (R_values G)

lemma lambda2_bounded_below (G : SimpleGraph α) [Fintype G.vertexSet]
    [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)] :
...
@[grind] noncomputable def SimpleGraph.energy (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)]
    (x : α → ℝ) : ℝ := ∑ e ∈ G.edgeFinset, Sym2.lift ⟨fun u v ↦ (x u - x v) ^ 2, by grind⟩ e

lemma energy_nonneg (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)] (x : α → ℝ) :
    0 ≤ G.energy x := by
  unfold SimpleGraph.energy; apply Finset.sum_nonneg; intro e he
  induction e using Sym2.ind; case h => dsimp; apply sq_nonneg

lemma energy_mul (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)] (x : α → ℝ) (c : ℝ) :
...
@[grind] noncomputable def SimpleGraph.deg_norm (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)]
    (x : α → ℝ) : ℝ := ∑ v ∈ G.vertexFinset, ↑(G.degree v) * x v ^ 2

lemma deg_norm_nonneg (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)] (x : α → ℝ) :
    0 ≤ G.deg_norm x := by
...
lemma deg_norm_eq_sum_reg (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)] (x : α → ℝ) (d : ℕ)
...
lemma deg_norm_mul (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)] (x : α → ℝ) (c : ℝ) :
    G.deg_norm (fun v => c * x v) = c ^ 2 * G.deg_norm x := by
  unfold SimpleGraph.deg_norm; rw [Finset.mul_sum]
  apply Finset.sum_congr rfl; grind

lemma deg_norm_shifted_parts_add (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)] (z : α → ℝ) :
...
lemma energy_shifted_parts_add_le (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)] (z : α → ℝ) :
...
lemma rQ_eq_energy_div_norm (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)] (x : α → ℝ) :
    G.rayleighQuotient x = G.energy x / G.deg_norm x := by grind

lemma rayleighQuotient_mul (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)] (x : α → ℝ) {c : ℝ}
...
@[grind] noncomputable def restrictToVertexSet (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)]
    [DecidableEq α] (x : α → ℝ) : α → ℝ := fun v => if v ∈ G.vertexFinset then x v else 0

lemma energy_restrictToVertexSet (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)]
...
lemma deg_norm_restrictToVertexSet (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)]
...
lemma rayleighQuotient_restrictToVertexSet (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)]
...
lemma orthogonalVectors_restrictToVertexSet (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)]
...
@[grind] def graphExpansionValidSubsets (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)] :
    Finset (Finset α) :=
  (G.vertexFinset.powerset).filter (fun S => S.Nonempty ∧ 2 * S.card ≤ (G.vertexFinset).card)

lemma mem_graphExpansionValidSubsets_of_valid (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)]
...
lemma graphExpansionValidSubsets_nonempty_of_valid (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)]
    (S : Finset α) (hS_ne : S.Nonempty) (hS_sub : S ⊆ G.vertexFinset)
...
lemma edgeExpansion_mem_graphExpansion_image_of_valid (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidablePred (· ∈ G.edgeSet)]
...
lemma edgeExpansion_zero_degree (G : SimpleGraph α)
    [Fintype G.vertexSet] [DecidableEq α] [DecidablePred (· ∈ G.edgeSet)]
