...
lemma sum_sq_pos (x : α → ℝ)
  ...
  apply lt_of_le_of_ne h_nonneg; intro h_sum_zero
...
lemma exists_ratio_le_of_sum_le {ι : Type*} (s : Finset ι) (a b : ι → ℝ) (R : ℝ)
    (hs : s.Nonempty) (hb_pos : ∀ i ∈ s, 0 < b i) (h_sum : ∑ i ∈ s, a i ≤ R * ∑ i ∈ s, b i) :
  ...
  have h_total_gt : R * ∑ i ∈ s, b i < ∑ i ∈ s, a i := by
    rw [Finset.mul_sum]; exact Finset.sum_lt_sum_of_nonempty hs h_each_gt
  nlinarith

lemma sum_le_sqrt_mul_sqrt_of_le_mul {ι : Type*} (s : Finset ι)
    (w A B : ι → ℝ) (hw_le : ∀ i ∈ s, w i ≤ A i * B i) :
    ∑ i ∈ s, w i ≤ Real.sqrt (∑ i ∈ s, A i ^ 2) * Real.sqrt (∑ i ∈ s, B i ^ 2) := by
  calc ∑ i ∈ s, w i ≤ ∑ i ∈ s, A i * B i := Finset.sum_le_sum hw_le
    _ ≤ Real.sqrt (∑ i ∈ s, A i ^ 2) * Real.sqrt (∑ i ∈ s, B i ^ 2) := by
      simpa using Real.sum_mul_le_sqrt_mul_sqrt s A B

lemma abs_sq_sub_sq_le_abs_sub_mul_add_of_nonneg {a b : ℝ} (ha : 0 ≤ a) (hb : 0 ≤ b) :
    |a ^ 2 - b ^ 2| ≤ |a - b| * (a + b) := by
  have hab_nonneg : 0 ≤ a + b := add_nonneg ha hb
  calc |a ^ 2 - b ^ 2| = |(a - b) * (a + b)| := by ring_nf
