...
/-!
# Executable finite-graph degree

Natural-valued degree is computed from an executable enumeration of the graph's actual edges.
In a general undirected graph, parallel actual edges count separately and each loop contributes
two: once through incidence and once through the loop correction. Directed loops contribute one
to each of out-degree and in-degree.
-/
...
contributes two. -/
def Graph.degree (G : Graph α β) (v : α) [Fintype E(G)] [DecidableEq α] : ℕ :=
  (G.incidenceFinset v).card + (G.loopFinset v).card

/-- The degree of `v` in a simple graph. -/
def SimpleGraph.degree (G : SimpleGraph α) (v : α)
    [Fintype E(G)] [DecidableEq α] : ℕ :=
...
theorem SimpleGraph.ncard_neighborSet_eq_degree
  ...
  classical
  let hn : (G.neighborSet v).Finite := Set.toFinite _
  rw [Set.ncard_eq_toFinset_card _ hn, ← G.card_incidenceFinset_eq_degree]
  apply Finset.card_bij (fun u _ => s(v, u))
  · intro u hu
    have hadj : G.Adj v u := hn.mem_toFinset.mp hu
    exact (G.mem_incidenceFinset v).mpr ⟨(G.adj_iff v u).mp hadj, by simp⟩
  · intro u hu w hw heq
    have huv : u ≠ v := (show G.Adj v u from hn.mem_toFinset.mp hu).ne.symm
    have hwv : w ≠ v := (show G.Adj v w from hn.mem_toFinset.mp hw).ne.symm
    simpa [Sym2.eq_iff, huv, hwv] using heq
  · intro e he
    have hinc := (G.mem_incidenceFinset v).mp he
    induction e with
    | h u w =>
      simp only [SimpleGraph.Inc, Sym2.mem_iff] at hinc
      rcases hinc.2 with rfl | rfl
      · refine ⟨w, hn.mem_toFinset.mpr ?_, rfl⟩
        exact (G.adj_iff v w).mpr hinc.1
      · refine ⟨u, hn.mem_toFinset.mpr ?_, Sym2.eq_swap⟩
        apply (G.adj_iff v u).mpr
        rw [show s(v, u) = s(u, v) from Sym2.eq_swap]
        exact hinc.1
...
@[simp] theorem DiGraph.outDegree_reverse (G : DiGraph α β) (v : α)
    ...
    G.reverse.outDegree v = G.inDegree v := by
...
@[simp] theorem DiGraph.inDegree_reverse (G : DiGraph α β) (v : α)
    [Fintype E(G)] [Fintype E(G.reverse)] [DecidableEq α] :
    G.reverse.inDegree v = G.outDegree v := by
...
@[simp] theorem Graph.degree_eq_zero_of_not_mem (G : Graph α β) (v : α)
    [Fintype E(G)] [DecidableEq α] (hv : v ∉ V(G)) : G.degree v = 0 := by
...
@[simp] theorem DiGraph.outDegree_eq_zero_of_not_mem (G : DiGraph α β) (v : α)
    [Fintype E(G)] [DecidableEq α] (hv : v ∉ V(G)) : G.outDegree v = 0 := by
...
@[simp] theorem DiGraph.inDegree_eq_zero_of_not_mem (G : DiGraph α β) (v : α)
    [Fintype E(G)] [DecidableEq α] (hv : v ∉ V(G)) : G.inDegree v = 0 := by
...
theorem SimpleGraph.degree_pos_iff_exists_adj (G : SimpleGraph α) (v : α)
    [Finite (G.neighborSet v)] [Fintype E(G)] [DecidableEq α] :
    0 < G.degree v ↔ ∃ u, G.Adj v u := by
  rw [← G.ncard_neighborSet_eq_degree]
  exact Set.ncard_pos (Set.toFinite _)
...
@[simp] theorem SimpleGraph.loopFinset_eq_empty (G : SimpleGraph α) (v : α)
    [Fintype E(G)] [DecidableEq α] : G.loopFinset v = ∅ := by
  ext e
  simp only [G.mem_loopFinset, Finset.notMem_empty, iff_false]
...
theorem Graph.degree_le_two_mul_card_edgeFinset (G : Graph α β) (v : α)
    [Fintype E(G)] [DecidableEq α] :
...
theorem Graph.degree_mono {H G : Graph α β} (hHG : H ≤ G) (v : α)
    [Fintype E(H)] [Fintype E(G)] [DecidableEq α] : H.degree v ≤ G.degree v := by
...
theorem SimpleGraph.degree_mono {H G : SimpleGraph α} (hHG : H ≤ G) (v : α)
    [Fintype E(H)] [Fintype E(G)] [DecidableEq α] : H.degree v ≤ G.degree v := by
  rw [← H.card_incidenceFinset_eq_degree, ← G.card_incidenceFinset_eq_degree,
    ← H.ncard_incidenceSet, ← G.ncard_incidenceSet]
  exact Set.ncard_le_ncard (fun _ h => hHG.inc h) (Set.toFinite _)

theorem DiGraph.outDegree_mono {H G : DiGraph α β} (hHG : H ≤ G) (v : α)
    [Fintype E(H)] [Fintype E(G)] [DecidableEq α] :
...
theorem Graph.degree_induce (G : Graph α β) (S : Set α) (v : α)
    [Fintype E(G.induce S)] [Fintype E(G)] [DecidableEq α] :
...
theorem Graph.degree_deleteEdges (G : Graph α β) (F : Set (Edge α β)) (v : α)
    [Fintype E(G.deleteEdges F)] [Fintype E(G)] [DecidableEq α] :
...
theorem Graph.degree_deleteEdge (G : Graph α β) (e : Edge α β) (v : α)
    [Fintype E(G.deleteEdge e)] [Fintype E(G)] [DecidableEq α] :
...
theorem Graph.degree_deleteVerts (G : Graph α β) (S : Set α) (v : α)
    [Fintype E(G.deleteVerts S)] [Fintype E(G)] [DecidableEq α] :
...
theorem Graph.degree_deleteVert (G : Graph α β) (u v : α)
    [Fintype E(G.deleteVert u)] [Fintype E(G)] [DecidableEq α] :
...
theorem Graph.degree_deleteEdgesBetween (G : Graph α β) (u w v : α)
    [Fintype E(G.deleteEdgesBetween u w)] [Fintype E(G)] [DecidableEq α] :
...
theorem DiGraph.outDegree_deleteEdge (G : DiGraph α β) (a : Arc α β) (v : α)
    [Fintype E(G.deleteEdge a)] [Fintype E(G)] [DecidableEq α] :
...
theorem DiGraph.outDegree_deleteArcsFromTo (G : DiGraph α β) (u w v : α)
    [Fintype E(G.deleteArcsFromTo u w)] [Fintype E(G)] [DecidableEq α] :
...
theorem SimpleGraph.exists_adj_of_nonempty_of_two_le_degree (G : SimpleGraph α)
    [Finite V(G)] [Fintype E(G)] [DecidableEq α] (hne : V(G).Nonempty)
...
private theorem Graph.vertexFinset_nonempty (G : Graph α β) [Fintype V(G)] [Nonempty V(G)] :
    G.vertexFinset.Nonempty := by
  let v : V(G) := Classical.choice (inferInstance : Nonempty V(G))
  exact ⟨v, G.mem_vertexFinset.mpr v.property⟩

private theorem SimpleGraph.vertexFinset_nonempty (G : SimpleGraph α) [Fintype V(G)]
    [Nonempty V(G)] : G.vertexFinset.Nonempty := by
  let v : V(G) := Classical.choice (inferInstance : Nonempty V(G))
  exact ⟨v, G.mem_vertexFinset.mpr v.property⟩

private theorem DiGraph.vertexFinset_nonempty (G : DiGraph α β) [Fintype V(G)] [Nonempty V(G)] :
    G.vertexFinset.Nonempty := by
  let v : V(G) := Classical.choice (inferInstance : Nonempty V(G))
  exact ⟨v, G.mem_vertexFinset.mpr v.property⟩

private theorem SimpleDiGraph.vertexFinset_nonempty (G : SimpleDiGraph α) [Fintype V(G)]
    [Nonempty V(G)] : G.vertexFinset.Nonempty := by
  let v : V(G) := Classical.choice (inferInstance : Nonempty V(G))
  exact ⟨v, G.mem_vertexFinset.mpr v.property⟩

/-- The maximum degree of a finite general graph; it is zero when the vertex set is empty. -/
def Graph.maxDegree (G : Graph α β) [Fintype V(G)] [Fintype E(G)] [DecidableEq α] : ℕ :=
  G.vertexFinset.sup fun v => G.degree v

/-- The minimum degree of a finite nonempty general graph. -/
def Graph.minDegree (G : Graph α β) [Fintype V(G)] [Fintype E(G)] [DecidableEq α]
    [Nonempty V(G)] : ℕ :=
...
def SimpleGraph.maxDegree (G : SimpleGraph α) [Fintype V(G)] [Fintype E(G)]
    [DecidableEq α] : ℕ :=
  G.vertexFinset.sup fun v => G.degree v

/-- The minimum degree of a finite nonempty simple graph. -/
def SimpleGraph.minDegree (G : SimpleGraph α) [Fintype V(G)] [Fintype E(G)]
    [DecidableEq α] [Nonempty V(G)] : ℕ :=
  G.vertexFinset.inf' G.vertexFinset_nonempty fun v => G.degree v

/-- The maximum out-degree of a finite general directed graph; it is zero when the vertex set is
empty. -/
def DiGraph.maxOutDegree (G : DiGraph α β) [Fintype V(G)] [Fintype E(G)]
    [DecidableEq α] : ℕ :=
  G.vertexFinset.sup fun v => G.outDegree v

/-- The minimum out-degree of a finite nonempty general directed graph. -/
def DiGraph.minOutDegree (G : DiGraph α β) [Fintype V(G)] [Fintype E(G)]
    [DecidableEq α] [Nonempty V(G)] : ℕ :=
...
empty. -/
def DiGraph.maxInDegree (G : DiGraph α β) [Fintype V(G)] [Fintype E(G)]
    [DecidableEq α] : ℕ :=
  G.vertexFinset.sup fun v => G.inDegree v

/-- The minimum in-degree of a finite nonempty general directed graph. -/
def DiGraph.minInDegree (G : DiGraph α β) [Fintype V(G)] [Fintype E(G)]
    [DecidableEq α] [Nonempty V(G)] : ℕ :=
...
empty. -/
def SimpleDiGraph.maxOutDegree (G : SimpleDiGraph α) [Fintype V(G)] [Fintype E(G)]
    [DecidableEq α] : ℕ :=
  G.vertexFinset.sup fun v => G.outDegree v

/-- The minimum out-degree of a finite nonempty simple directed graph. -/
def SimpleDiGraph.minOutDegree (G : SimpleDiGraph α) [Fintype V(G)] [Fintype E(G)]
    [DecidableEq α] [Nonempty V(G)] : ℕ :=
  G.vertexFinset.inf' G.vertexFinset_nonempty fun v => G.outDegree v

/-- The maximum in-degree of a finite simple directed graph; it is zero when the vertex set is
empty. -/
def SimpleDiGraph.maxInDegree (G : SimpleDiGraph α) [Fintype V(G)] [Fintype E(G)]
    [DecidableEq α] : ℕ :=
  G.vertexFinset.sup fun v => G.inDegree v

/-- The minimum in-degree of a finite nonempty simple directed graph. -/
def SimpleDiGraph.minInDegree (G : SimpleDiGraph α) [Fintype V(G)] [Fintype E(G)]
    [DecidableEq α] [Nonempty V(G)] : ℕ :=
  G.vertexFinset.inf' G.vertexFinset_nonempty fun v => G.inDegree v

/-! ### Undirected extrema API -/

theorem Graph.degree_le_maxDegree (G : Graph α β)
...
theorem Graph.maxDegree_le_of_forall_degree_le (G : Graph α β)
    [Fintype V(G)] [Fintype E(G)] [DecidableEq α]
    {d : ℕ} (h : ∀ v, v ∈ V(G) → G.degree v ≤ d) : G.maxDegree ≤ d :=
  Finset.sup_le fun v hv => h v (G.mem_vertexFinset.mp hv)

theorem Graph.exists_maxDegree_eq_degree (G : Graph α β)
    [Fintype V(G)] [Fintype E(G)] [DecidableEq α]
...
theorem Graph.exists_minDegree_eq_degree (G : Graph α β)
    [Fintype V(G)] [Fintype E(G)] [DecidableEq α]
...
theorem Graph.minDegree_le_maxDegree (G : Graph α β)
    [Fintype V(G)] [Fintype E(G)] [DecidableEq α]
...
theorem SimpleGraph.degree_le_maxDegree (G : SimpleGraph α)
    [Fintype V(G)] [Fintype E(G)] [DecidableEq α]
    {v : α} (hv : v ∈ V(G)) : G.degree v ≤ G.maxDegree :=
  Finset.le_sup (f := fun v => G.degree v) (G.mem_vertexFinset.mpr hv)

theorem SimpleGraph.maxDegree_le_of_forall_degree_le (G : SimpleGraph α)
    [Fintype V(G)] [Fintype E(G)] [DecidableEq α]
    {d : ℕ} (h : ∀ v, v ∈ V(G) → G.degree v ≤ d) : G.maxDegree ≤ d :=
  Finset.sup_le fun v hv => h v (G.mem_vertexFinset.mp hv)

theorem SimpleGraph.exists_maxDegree_eq_degree (G : SimpleGraph α)
    [Fintype V(G)] [Fintype E(G)] [DecidableEq α]
    [Nonempty V(G)] : ∃ v, v ∈ V(G) ∧ G.maxDegree = G.degree v := by
  obtain ⟨v, hv, hmax⟩ := Finset.exists_mem_eq_sup G.vertexFinset
    G.vertexFinset_nonempty (fun v => G.degree v)
...
theorem SimpleGraph.minDegree_le_degree (G : SimpleGraph α)
    [Fintype V(G)] [Fintype E(G)] [DecidableEq α] [Nonempty V(G)]
    {v : α} (hv : v ∈ V(G)) : G.minDegree ≤ G.degree v :=
  Finset.inf'_le (f := fun v => G.degree v) (G.mem_vertexFinset.mpr hv)

theorem SimpleGraph.le_minDegree_of_forall_le_degree (G : SimpleGraph α)
    [Fintype V(G)] [Fintype E(G)] [DecidableEq α]
    [Nonempty V(G)] {d : ℕ} (h : ∀ v, v ∈ V(G) → d ≤ G.degree v) :
    d ≤ G.minDegree :=
  Finset.le_inf' G.vertexFinset_nonempty (fun v => G.degree v) fun v hv =>
...
theorem SimpleGraph.exists_minDegree_eq_degree (G : SimpleGraph α)
    [Fintype V(G)] [Fintype E(G)] [DecidableEq α]
    [Nonempty V(G)] : ∃ v, v ∈ V(G) ∧ G.minDegree = G.degree v := by
  obtain ⟨v, hv, hmin⟩ := Finset.exists_mem_eq_inf' G.vertexFinset_nonempty
    (fun v => G.degree v)
...
theorem SimpleGraph.minDegree_le_maxDegree (G : SimpleGraph α)
    [Fintype V(G)] [Fintype E(G)] [DecidableEq α] [Nonempty V(G)] :
...
theorem DiGraph.outDegree_le_maxOutDegree (G : DiGraph α β)
    [Fintype V(G)] [Fintype E(G)] [DecidableEq α]
...
theorem DiGraph.maxOutDegree_le_of_forall_outDegree_le (G : DiGraph α β)
    [Fintype V(G)] [Fintype E(G)] [DecidableEq α] {d : ℕ}
...
theorem DiGraph.le_minOutDegree_of_forall_le_outDegree (G : DiGraph α β)
    [Fintype V(G)] [Fintype E(G)] [DecidableEq α] [Nonempty V(G)] {d : ℕ}
...
theorem DiGraph.inDegree_le_maxInDegree (G : DiGraph α β)
    [Fintype V(G)] [Fintype E(G)] [DecidableEq α]
...
theorem SimpleDiGraph.outDegree_le_maxOutDegree (G : SimpleDiGraph α)
    [Fintype V(G)] [Fintype E(G)] [DecidableEq α]
    {v : α} (hv : v ∈ V(G)) : G.outDegree v ≤ G.maxOutDegree :=
  Finset.le_sup (f := fun v => G.outDegree v) (G.mem_vertexFinset.mpr hv)

theorem SimpleDiGraph.maxOutDegree_le_of_forall_outDegree_le
    (G : SimpleDiGraph α) [Fintype V(G)] [Fintype E(G)] [DecidableEq α] {d : ℕ}
    (h : ∀ v, v ∈ V(G) → G.outDegree v ≤ d) : G.maxOutDegree ≤ d :=
  Finset.sup_le fun v hv => h v (G.mem_vertexFinset.mp hv)
...
theorem SimpleDiGraph.inDegree_le_maxInDegree (G : SimpleDiGraph α)
    [Fintype V(G)] [Fintype E(G)] [DecidableEq α]
    {v : α} (hv : v ∈ V(G)) : G.inDegree v ≤ G.maxInDegree :=
  Finset.le_sup (f := fun v => G.inDegree v) (G.mem_vertexFinset.mpr hv)
