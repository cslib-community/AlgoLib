...
A structurally recursive `GraphLib.List.commonPrefix`, together with the single splitting
...
The declaration lives in a GraphLib-owned list namespace so this project does
not extend Mathlib's root `List` namespace.

## Main definitions

* `GraphLib.List.commonPrefix` — the longest common prefix of two lists.

## Main results

* `GraphLib.List.commonPrefix_split` — the common prefix cuts both lists, and the first
