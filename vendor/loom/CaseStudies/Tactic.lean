/- AlgoLib port of upstream Loom: SMT/oracle and async-admission paths removed.
   See vendor/README.md for provenance and compatibility changes. -/
import Lean
import Lean.Parser
...
/- AlgoLib port: only proof-producing native tactics; no trusted SMT. -/
elab_rules : tactic
  | `(tactic| loom_auto) => do
    evalTactic (← `(tactic| first | assumption | omega | grind))
...
elab_rules : tactic
  | `(tactic| $vls:loom_solve_tactic) => withMainContext do
    ...
    if let `(loom_solve_tactic| loom_solve?) := vls then
      ...
      evalTactic vlsIntro
      if (loomSolveAsync? vls).isSome then
        throwError "Async admission is disabled: use loom_solve for kernel-checked proofs"
...
elab_rules : tactic
  | `(tactic| loom_solver) => withMainContext do
    ...
    match solver with
      ...
      | _ => throwError "SMT backends are excluded in this port; use loom.solver grind or custom"
