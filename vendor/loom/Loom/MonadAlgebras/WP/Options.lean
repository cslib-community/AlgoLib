/- AlgoLib port: see vendor/README.md for compatibility and trust-policy changes. -/
import Lean
import Lean.Parser

/- AlgoLib Lean 4.30 compatibility: typed option accessors. -/
def Lean.Options.getString (o : Lean.Options) (k : Lean.Name) (defVal : String := "") : String :=
  o.get k defVal
def Lean.Options.getNat (o : Lean.Options) (k : Lean.Name) (defVal : Nat := 0) : Nat :=
  o.get k defVal
...
register_option loom.solver : LoomSolver := {
  ...
  "Loom solver options. Right now we support
   - `grind` to use proof-producing native Lean tactics
   - SMT options are retained for parsing compatibility but rejected by this port
