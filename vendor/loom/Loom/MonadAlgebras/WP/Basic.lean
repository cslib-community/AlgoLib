/- AlgoLib port: Lean 4.30 chain-indexed suprema and legacy range compatibility. -/
...
instance [md : Monad m] [ccpo : ∀ α, CCPO (m α)] [mono : MonoBind m] : ForIn m Lean.Loop Unit where
  forIn xs init f := Loop.forIn xs init f
...
lemma wp_bot :
  ...
  have h := wp_csup (m := m) (∅ : Set (m α)) post (chain_empty _)
  simpa only [Set.mem_empty_iff_false, iInf_of_empty, iInf_top, iInf_const] using h
...
lemma repeat_inv (f : Unit -> β -> m (ForInStep β))
  ...
  { apply Lean.Order.admissible_pi_apply
    ...
    apply le_trans (le_iInf₂ fun c hc => h c hc) (wp_csup loops _ cl) }
...
lemma wp_except_handler_eq ε (hd : ε -> Prop) [IsHandler hd] (c : ExceptT ε m α) post :
  wp c post = wp (m := m) c (fun | .ok x => post x | .error e => ⌜hd e⌝) := by
    change MAlg.lift c post = _
    rw [MAlg.lift_ExceptT]
    change MAlg.lift (m := m) c (Except.post hd post) = MAlg.lift (m := m) c _
    congr 1
    funext e
    cases e <;> rfl
...
lemma TotalCorrectness.DivM.wp_eq (α : Type) (x : DivM α) (post : α -> Prop) :
  wp x post =
    ...
    | .div => False
    | .res r => post r := by
  simp [wp, liftM, monadLift, MAlg.lift, Functor.map, MAlgOrdered.μ, TotalCorrectness.instMAlgOrderedDivMProp]
...
lemma PartialCorrectness.DivM.wp_eq (α : Type) (x : DivM α) (post : α -> Prop) :
  wp x post =
    ...
    | .div => True
    | .res r => post r := by
  simp [wp, liftM, monadLift, MAlg.lift, Functor.map, MAlgOrdered.μ, PartialCorrectness.instMAlgOrderedDivMProp]
  cases x <;> simp
