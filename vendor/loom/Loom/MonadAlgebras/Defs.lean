module
/- AlgoLib port: Lean 4.30 CCPO and explicit transformer-equality proofs. -/
...
@[app_unexpander LE.pure] meta def unexpandPure : Lean.PrettyPrinter.Unexpander
  | `($(_) $p:term) => `(⌜$p:term⌝)
  | _ => throw ()
...
class MAlgPartial (m : Type u -> Type v) [Monad m] [∀ α, Lean.Order.CCPO (m α)]
  ...
  csup_lift {α : Type u} (xc : Set (m α)) (post : α -> l) :
    (h : Lean.Order.chain xc) ->
    ⨅ x ∈ xc, MAlg.lift x post <= MAlg.lift (Lean.Order.CCPO.csup h) post
...
@[reducible] def LogicLift.refl [CompleteLattice l] : LogicLift l l where
  lift := by exact ⟨id⟩
  lift_top := rfl
  lift_bot := rfl
...
instance [CompleteLattice l] [CompleteLattice k] [CompleteLattice p] [linst₁ : LogicLiftT l k] [linst₂ : LogicLift k p] : LogicLiftT l p where
  lift_top := by
    intro α
    change linst₂.lift.monadLift (linst₁.lift.monadLift _) = _
...
@[reducible] def MAlgLift.mk_rep  (m : semiOutParam (Type u -> Type v)) (l : semiOutParam (Type u)) [Monad m] [CompleteLattice l] [MAlgOrdered m l]
...
@[reducible] def MAlgLift.mk_id  (m : semiOutParam (Type u -> Type v)) (l : semiOutParam (Type u)) [Monad m] [CompleteLattice l] [MAlgOrdered m l]
...
instance MAlgLiftTTrans
  ...
  MAlgLiftT m l p q where
    ...
    μ_lift := by
      ...
      erw [inst.μ_lift]
      apply congrArg (fun z => inst.cl.lift.monadLift z _)
      funext post
      exact inst'.μ_lift _
